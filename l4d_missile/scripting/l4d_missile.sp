...
{
	g_hCvarMissileRadius = CreateConVar("l4d_missile_radius", "200.0", "Missile explode radius", FCVAR_NOTIFY);
	g_hCvarMissileDamage = CreateConVar("l4d_missile_damage", "500.0", "Damage done by missile");
	g_hCvarMissileDamageToSurvivor = CreateConVar("l4d_missile_damage_tosurvivor", "0.0", "Damage to survivors", FCVAR_NOTIFY);
	g_hCvarMissilePush = CreateConVar("l4d_missile_push", "1200", "Push force done to target", FCVAR_NOTIFY);

	g_hCvarMissileSafe = CreateConVar("l4d_missile_safe", "1", "0:Normal chance of damage to survivor, 1:Less chance to hurt survivor [0, 1]", FCVAR_NOTIFY);

	g_hCvarMissileSmoker = CreateConVar("l4d_missile_infected_smoker", "0.0", "Launch missile when smoker drags [0.0, 30.0]%", FCVAR_NOTIFY);
	g_hCvarMissileCharger = CreateConVar("l4d_missile_infected_charger", "0.0", "Launch missile when charger charges [0.0, 30.0]%", FCVAR_NOTIFY);
	g_hCvarMissileSpitter = CreateConVar("l4d_missile_infected_spitter", "0.0", "Launch missile when spitter spits [0.0, 30.0]%", FCVAR_NOTIFY);
	g_hCvarMissileWitch = CreateConVar("l4d_missile_infected_witch", "0.0", "Launch missile when witch is startled[0.0, 30.0]%");
	g_hCvarMissileTankThrow = CreateConVar("l4d_missile_infected_tank_throw", "0.0", "Launch missile when tank throws rock[0.0, 30.0]%", FCVAR_NOTIFY);
	g_hCvarMissileCommon = CreateConVar("l4d_missile_infected_anti", "0.0", "Common infected launch missile when survivor launch missile [0.0, 30.0]%", FCVAR_NOTIFY);

	g_hCvarMissileRifle = CreateConVar("l4d_missile_weapon_rifle", "1", "Enable or disable missiles for rifles {0, 1}", FCVAR_NOTIFY);
	g_hCvarMissileSniper = CreateConVar("l4d_missile_weapon_sniper", "1", "Enable or disable missiles for snipers {0, 1}", FCVAR_NOTIFY);
	g_hCvarMissileShotgun = CreateConVar("l4d_missile_weapon_shotgun", "1", "Enable or disable missiles for shotguns {0, 1}", FCVAR_NOTIFY);
	g_hCvarMissileSmg = CreateConVar("l4d_missile_weapon_smg", "1", "Enable or disable missiles for smgs {0, 1}", FCVAR_NOTIFY);
	g_hCvarMissilePistols = CreateConVar("l4d_missile_weapon_pistols", "1", "Enable or disable missiles for pistols & magnum {0, 1}", FCVAR_NOTIFY);
	g_hCvarMissileGrenadeLauncher = CreateConVar("l4d_missile_weapon_grenade", "1", "Enable or disable missiles for grenade launcher {0, 1}", FCVAR_NOTIFY);

	g_hCvarMissileLimit = CreateConVar("l4d_missile_limit", "3", "Amount of missiles you can carry", FCVAR_NOTIFY);
	g_hCvarMissileKills = CreateConVar("l4d_missile_kills", "30", "How many infected killed rewards one missile", FCVAR_NOTIFY);
	g_hCvarMissileTraceFactor = CreateConVar("l4d_missile_tracefactor", "1.5", "Trace factor of missile. Do not need to change [0.5, 3.0]", FCVAR_NOTIFY);
	g_hCvarMissileRadarRange = CreateConVar("l4d_missile_radar_range", "1500.0", "Radar scan range: missiles do not lock on target if out of this range [500.0, -]", FCVAR_NOTIFY);

	g_hCvarMinimalMode = CreateConVar("l4d_missile_minimal_mode", "1", "reduce visual clutter (chat)", FCVAR_NOTIFY);
...
void UpGrade(int x, int kill)
{
	...
	{
		PrintHintText(x, "[ MISSILES ( %d/%d ) ]", MissileCount[x], g_iMissileLimit);
		...
		else if (g_iMinimalMode <= 0)
		{
			if(ShowMsg[x] <= g_iMissileLimit)
			{
				CPrintToChat(x, "{green}[{lightgreen}TIP!{green}] {default}Press {green}E {default}+ {green}MOUSE1 {default}to fire a missile with any weapon,\n{green}CTRL {default}+ {green}E {default}+ {green}M1 {default}to lock onto a target.", MissileCount[x]);
				ShowMsg[x]++;
			}
...
{
	time = time + 0.0; //to prevent the compiler from warning, relevant to "type += 0"
	...
	if(MissileCount[client] - 1 >= 0)
	{
		...
		PrintHintText(client, "You have no missiles! Get %d kills to earn one.", g_iMissileKills);
}
...
bool LaunchMissile(int client, float force, int type = MissileTrace, bool up = false, float offset)
{
	...
	PrintHintText(client, "Missile Reserve: %d/%d", MissileCount[client], g_iMissileLimit);
...
{
	bool ok = false;
	type = type + 0; //to prevent the compiler from warning, relevant to "type += 0"
		...
		if(show)
		{
			if(enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))
			{
				if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
					PrintHintText(enemy, "Missile Locked on <%N>, distance: %d", client, RoundFloat(disenemy) );

				else
					PrintHintText(enemy, "WARNING! Enemy's missile locked, distance: %d", RoundFloat(disenemy) );
			...
			{
				if(enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))
					PrintHintText(client, "Missile Locked on <%N>, distance: %d", enemy, RoundFloat(disenemy));

				else
					PrintHintText(client, "Missile Locked on enemy, distance: %d", RoundFloat(disenemy));
			}
		}
	}
		...
		if(show)
		{
			if(client > 0 && IsClientInGame(client) && IsPlayerAlive(client))
			{
				if(enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))
					...
					PrintHintText(client, "Missile locked on enemy's missile, distance: %d", RoundFloat(disenemy));
			}
		}
		enemyismissile = true;
	}
...
void MissileHitMissileMsg(int client, int enemy, bool hit = true)
{
	if(hit)
	{
		if(enemy > 0 && IsClientInGame(enemy))
		{
			if(client > 0 && IsClientInGame(client))
				PrintHintText(enemy, "FAIL! Your missile was intercepted by: %N.", client);

			else
				PrintHintText(enemy, "FAIL! Your missile was intercepted by enemy.");
		}

		if(client > 0 && IsClientInGame(client))
		{
			if(enemy > 0 && IsClientInGame(enemy) && IsPlayerAlive(enemy))
				PrintHintText(client, "SUCCESS! You intercepted %N's missile.", enemy);

			else
				PrintHintText(client, "SUCCESS! You intercepted the enemy's missile.");
		}
	}
	...
	{
		if(client > 0 && IsClientInGame(client))
			PrintHintText(client, "FAIL! Missile Missed.");
	}
}
...
void MissileHitPlayerMsg(int client, int enemy, bool hit)
{
	if(hit)
	{
		...
		{
			if(client > 0 && IsClientInGame(client))
				PrintHintText(enemy, "You were hit by %N's missile!", client);

			else
				PrintHintText(enemy, "You were hit by the enemy's missile.");
		}

		if(client > 0 && IsClientInGame(client))
		{
			if(enemy > 0 && IsClientInGame(enemy))
				PrintHintText(client, "SUCCESS! Your missile hit: %N", enemy);

			else
				PrintHintText(client, "FAIL! Your missile hit a obstacle.");
		}
	}
	...
	{
		if(client > 0 && IsClientInGame(client))
			PrintHintText(client, "FAIL! Your missile missed.");
	}
}
...
{
	{
		...
		if(MissileType[client] != MissileTrace && IsClientInGame(client) && IsPlayerAlive(client))
			PrintHintText(client, "SUCCESS! Missile Exploded");
	}
}
