...
int SND_Looped;
bool g_bSNDLoopedStart, g_bSNDLoopedEnd;
int CommonBombTypeIndex;
int dev;
int INFECTED_FLAG_CANT_SEE_SURVIVORS, INFECTED_FLAG_CANT_HEAR_SURVIVORS, INFECTED_FLAG_CANT_FEEL_SURVIVORS;


int usewav;

//doesnt work as of now, dont enable
//int entSound;

#define SOUND_KAMIKAZE "droS43/l4d_mutant_zombies_bomb_attackkamikaze.mp3"

//doesnt work as of now, dont enable
//#define SOUND_KAMIKAZE_WAV "droS43/l4d_mutant_zombies_bomb_attackkamikaze.wav"
...
public void OnMapStart()
{
	...
	if(!usewav)
		PrecacheSound(SOUND_KAMIKAZE, true);

	//doesnt work as of now, dont enable
	//else if(usewav)
		//PrecacheSound(SOUND_KAMIKAZE_WAV, true);
...
{
	if(g_iLoadStatus == 0 || g_bCvarAllow == false)
		return;
...
void SND_Looped_End()
{	
	if(g_bSNDLoopedEnd == true)
	{
		if(dev)
			PrintToServer("[L4D Mutants] Common Infected (Bomb) Sound Loop Ended");
	}

	return;
}

void SND_Looped_Start()
{	
	if(g_bSNDLoopedEnd == false || !g_bSNDLoopedEnd || CommonBombTypeIndex != 1)
	{
		g_bSNDLoopedStart = true;
	}

	else if(g_bSNDLoopedEnd == true)
		g_bSNDLoopedStart = false;

	if(g_bSNDLoopedStart == true)
	{
		if(dev)
			PrintToServer("[L4D Mutants] Common Infected (Bomb) Sound Loop Started");
	}
...
//doesnt work as of now, dont enable
/*
public void CreateSound(int common)
{
	entSound = CreateEntityByName("ambient_generic");

	//char sndtargetname[128];
	//Format(sndtargetname, sizeof(sndtargetname), "snd_bomb_%i", common);

	char bombtargetname[128];
	Format(bombtargetname, sizeof(bombtargetname), "bomb_%i", common);
	DispatchKeyValue(common, "targetname", bombtargetname); //infected dont contain a targetname (m_iName) network property so this generetes one (each uniquely) for them.

	DispatchKeyValue(entSound, "targetname", bombtargetname); //sets the ambient_generic's targetname
    DispatchKeyValue(entSound, "message", SOUND_KAMIKAZE_WAV);
    DispatchKeyValue(entSound, "volume", "10");
    DispatchKeyValue(entSound, "radius", "2000");
   	DispatchKeyValue(entSound, "SourceEntityName", bombtargetname); //sets the ambient_generic to emit from this entity
    //DispatchKeyValue(entSound, "spawnflags", "32");
    DispatchKeyValue(entSound, "spawnflags", "49");

    DispatchSpawn(entSound);

    //ActivateEntity(entSound);
    //SetVariantString("!self");
    //AcceptEntityInput(entSound,"AddOutput");
    AcceptEntityInput(entSound, "PlaySound");
    //AcceptEntityInput(entSound, "ToggleSound");

    if(dev)
    {
    	int flags = GetEntProp(entSound, Prop_Data, "m_spawnflags");
    	PrintToServer("[L4D Mutants] Bomb SFX Post-Spawn Flags: %d", flags);

    	char targetname[128];
		GetEntPropString(entSound, Prop_Data, "m_iName", targetname, sizeof(targetname));
		PrintToServer("[L4D Mutants] Bomb SFX TargetName: %s", targetname);

		char targetname2[128];
		GetEntPropString(g_iInfectedBomb[CommonBombTypeIndex][0], Prop_Data, "m_iName", targetname2, sizeof(targetname2));
		PrintToServer("[L4D Mutants] Bomb TargetName: %s", targetname2);
    }
}
*/
...
//					BOMB
// ====================================================================================================
public void MutantBombSetup(int common)
{
	//common2 = common;

	g_bHookCommonBomb = false;

	int index = GetEntityIndex(TYPE_BOMB); //worldspawn

	if(index == -1)
		return;

	CommonBombTypeIndex = index;

	g_iInfectedBomb[CommonBombTypeIndex][0] = EntIndexToEntRef(common); //infected

	//fuckyou = g_iInfectedBomb[CommonBombTypeIndex][0] = EntIndexToEntRef(common);
	...
	//network properties
	- m_usSolidFlags (type integer)
	- m_hOwnerEntity (type integer)
	- m_hScriptUseTarget (type integer)
	- m_flNextAttack (type float)
	- m_viewtarget (type vector)
	- m_fFlags (type integer)
	- m_nFallenFlags (type integer)
	- m_hActiveWeapon (type integer)
	- movetype (type integer)

		Table: baseclass (offset 0) (type DT_NextBot)
	...
	NB_BEHAVIOR
	InfectedExecAction( InfectedBehavior( InfectedAlert ) )
	InfectedExecAction( InfectedBehavior( InfectedAttack( ChaseVictim ) ) )

	ent id: 255
	ent flags: 73728
	ent m_fFlags: 8192
	ent m_iClassname: infected

	SetEntProp(index, _, _, _);
	GetEntProp(index, _, _);
	GetEntityFlags(_)
	DispatchKeyValue(index, "", "");

	*/

	int DirectorOptions_InfectedFlags = GetEntProp(common, Prop_Send, "m_fFlags");

	if(DirectorOptions_InfectedFlags)																
	{	
		if(dev)
			PrintToServer("[L4D Mutants] Common Infected (Bomb) Behavior Flags (m_fFlags): %d", DirectorOptions_InfectedFlags);

		int exception_index;

		if(DirectorOptions_InfectedFlags == INFECTED_FLAG_CANT_SEE_SURVIVORS)
			exception_index = 1;

		else if(DirectorOptions_InfectedFlags == INFECTED_FLAG_CANT_HEAR_SURVIVORS)
			exception_index = 2;

		else if(DirectorOptions_InfectedFlags == INFECTED_FLAG_CANT_FEEL_SURVIVORS)
			exception_index = 3;

		else if(DirectorOptions_InfectedFlags == INFECTED_FLAG_CANT_SEE_SURVIVORS + INFECTED_FLAG_CANT_HEAR_SURVIVORS + INFECTED_FLAG_CANT_FEEL_SURVIVORS)
			exception_index = 4;
		...
		switch(exception_index)
		{
			...
			{	
				if(dev)
					PrintToServer("[L4D Mutants] SUCCESS: COMMON INFECTED (Bomb) CAN SEE, HEAR OR FEEL SURVIVORS!");
			}
			case 1:
			{	
				if(dev)
					PrintToServer("[L4D Mutants] EXCEPTION: COMMON INFECTED (Bomb) CAN'T SEE SURVIVORS!");
			}
			case 2:
			{	
				if(dev)
					PrintToServer("[L4D Mutants] EXCEPTION: COMMON INFECTED (Bomb) CAN'T HEAR SURVIVORS!");
			}
			case 3:
			{
				if(dev)
					PrintToServer("[L4D Mutants] EXCEPTION: COMMON INFECTED (Bomb) CAN'T FEEL SURVIVORS!");
			}
			case 4:
			{
				if(dev)
					PrintToServer("[L4D Mutants] EXCEPTION: COMMON INFECTED (Bomb) CAN'T SEE, HEAR OR FEEL SURVIVORS!");
			}
	...
	if(dev)
		PrintToServer("[L4D Mutants] Common Infected (Bomb) EntID: %d", common);

	if(!usewav)
	{	
		//0.76 is not perfect but good enough, anything below just sucks
		RTimerCreate(0.76, SoundLoop, EntIndexToEntRef(common), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE | TIMER_FLAG_NO_ROUNDCHANGE);
		SND_Looped_Start();
	}
	...
	{
		CreateSound(common);
		CreateTimer(0.1, AliveCheck, EntIndexToEntRef(common), TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	}
	*/	
}
...
//doesnt work as of now, dont enable
/*
public Action AliveCheck(Handle timer, any common)
{	
	common = EntRefToEntIndex(common);

	if(common == INVALID_ENT_REFERENCE || CommonBombTypeIndex == -1)
	{
		if(dev)
			PrintToServer("[L4D Mutants] ERROR: COMMON INFECTED (Bomb) ENT INVALID! STOPPING...");

		RemoveEdict(entSound);
	...
	{
		//if(dev)
			//PrintToServer("[L4D Mutants] SUCCESS: COMMON INFECTED (Bomb) ENT VALID!");
	}
...
public Action SoundLoop(Handle timer, any common)
{	
	common = EntRefToEntIndex(common);
...
void SND_Kamikaze(any common)
{
	if (SND_Looped < 0) 
        return;

    EmitSoundToAll(SOUND_KAMIKAZE, common, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, _, _, true, 0.0);
	SND_Looped++;

	//if(dev)
		//PrintToServer("snd looped: %d", SND_Looped);
}
...
void MutantMindSetup(int common)
{
	...
	CreateCorrection2(common, index);
}
...
//this removes valve map color corrections when this is executed, possibly due to a engine limit of only 1 being active at a time.
...
void CreateCorrection2(int common, int index)
{
	float vPos[3];
...
int GetEntityIndex(int type)
{
	...
	for(int i = 0; i < MAX_ENTS; i++)
	{
		if(g_iConfLimit == 0)
		{
			if((type == TYPE_BOMB && IsValidEntRef(g_iInfectedBomb[i][0])) || (type == TYPE_FIRE && IsValidEntRef(g_iInfectedFire[i][0])) || (type == TYPE_GHOST && IsValidEntRef(g_iInfectedGhost[i])) || (type == TYPE_MIND && IsValidEntRef(g_iInfectedMind[i][0])) || (type == TYPE_SMOKE && IsValidEntRef(g_iInfectedSmoke[i][0])) || (type == TYPE_SPIT && IsValidEntRef(g_iInfectedSpit[i][0])) || (type == TYPE_TESLA && IsValidEntRef(g_iInfectedTesla[i][0])))
			{
				count++;
			...
			else if(index == -1)
			{
				...
				if(limit == 0)
					break; // No point going through all arrays if no set limit, lets save that extra handful of CPU cycles
			}
		}
...
bool IsCommonValidToUse(int entity)
{
	...
	if(!g_iConfUncommon && g_bLeft4Dead2)
	{
		...
		if(strcmp(sTemp, "models/infected/common_male_") == 0 && (strcmp(sTemp[28], "ceda.mdl") == 0 || strcmp(sTemp[28], "clown.mdl") == 0 || strcmp(sTemp[28], "fallen_survivor.mdl") == 0 || strcmp(sTemp[28], "jimmy.mdl") == 0 || strcmp(sTemp[28], "mud.mdl") == 0 || strcmp(sTemp[28], "riot.mdl") == 0 || strcmp(sTemp[28], "roadcrew.mdl") == 0))
			return false;
	}
